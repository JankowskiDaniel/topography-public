...
def _generate_noise_image(
    ...
    used_raw_images: npt.NDArray | None = None,
    seed: int | None = None,
    ...
    :param num_used_raw_images: Number of raw images used to extract one noise,
    defaults to 100
    :type num_used_raw_images: int, optional
    :param path_to_raw: Path to raw images, defaults to None
    :type path_to_raw: str, optional
    :param used_raw_images: Indices of raw images used for extracting noise,
    defaults to None
    ...
    # this part is used only when we generate single noise image by this
    # function and we don't use generate_average_noise_dataset function.
    # TODO: For sure it's require review and optimization.
    ...
    for i in range(num_used_raw_images):
        ...
        if path_to_raw:
            ...
            raw_filename = pkg_resources.resource_filename(
                __name__,
                f"/samples/raw/{str(used_raw_images[i]).zfill(5)}.png",  # type: ignore  # noqa: E501
            )
        img = cv2.imread(raw_filename)
        img = cv2.resize(img, size, interpolation=cv2.INTER_AREA)[:, :, 0]
        noise_image = noise_image + img  # type: ignore
...
def generate_average_noise_dataset(
    ...
    """Create noise dataset. The way how seed work is as follows:
        1. We count how many raw images are available. If path_to_raw was
        not passed (available only when you installed package via pip)
            then this number is equal to 300 (package has 300 raw images
            installed, something like example datasets in scikit-learn).
            In case when path to raw was passed, then we count how many files
            are in that directory, therefore there should be only raw images
            and any other files. Moreover, raw images in that directory should
            be named like in the original dataset, always from 0-N with zeros
            at the beginning of the filename, so that in result filename
            contains always 5 chars. This is becuase for know the code assumes
            that for example if directory contains 200 images, then we have
            there 200 images named from 00000.png, 00001.png, ..., 00199.png.
            IT WON'T WORK IN CASE OF ANY OTHER FILES ORGANIZATION.
        2. Numpy generates random list of images indices which will be used to
        extract noise, if you use seed, this list will be same all the time.
    ...
    :param num_used_raw_images: Raw images used to extract one noise,
    defaults to 100
    :type num_used_raw_images: int, optional
    :param path_to_raw: Path to raw images, defaults to None
    :type path_to_raw: str, optional
