...
class ImageDetails(BaseModel):
    ...
    def _check_width_center(cls, width_center, values, **kwargs) -> None:
        ...
        if width_center < min_width_center or width_center > max_width_center:
            raise ValueError(
                f"Width of the image center must be in range <{min_width_center};{max_width_center}> (up to 5% shift from the default center)"
            )
        return width_center
    ...
    def _check_height_center(cls, height_center, values, **kwargs) -> None:
        ...
        if height_center < min_height_center or height_center > max_height_center:
            raise ValueError(
                f"Height of the image center must be in range <{min_height_center};{max_height_center}> (up to 5% shift from the default center)"
            )
