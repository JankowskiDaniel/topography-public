...
class AnalyticalMathod:
    ...
    def detect_middle_x(self, img, filter_size=10, check=[], mode="x"):
        ...
        distances = np.array(
            [-maxima[i] + maxima[i + 1] for i in range(len(maxima) - 1)]
        )
    ...
    def calculate_epsilon(self, img, check=[], blur_size=20, vector_filter_size=10):
        ...
        if len(right_distances) > len(left_distances):
            longer_sequence = right_distances
        ...
        # get rid of it with some simple rules
        ...
        epsilon = np.fabs(1 + intercept / slope) % 1.0
