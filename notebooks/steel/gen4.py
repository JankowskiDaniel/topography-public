...
def generate_perlin_noise(
    width, height, scale=100, octaves=6, persistence=0.7, lacunarity=2.0
):
...
avg_params = avg_params.iloc[22500:30000]
...
for index, row in tqdm(avg_params.iterrows()):
    ...
    blended_image = apply_clahe(
        blended_image, tile_grid_size=grid_size, clip_limit=clip_limit
    )
    blended_image = controller.generate(blended_image, row["epsilon"])
    filename = f"perlin_moved_{counter:05d}.png"
...
df.to_csv(path_to_save + "parameters4.csv", index=False)
