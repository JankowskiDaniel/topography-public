...
def plot_epsilon_distribution(df, title):
    plt.hist(
        df["epsilon"], bins=10, range=(0, 1), color="red", alpha=0.5, edgecolor="black"
    )
    plt.title(title)
    plt.xlabel("Epsilon")
...
def generate_merged_df(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    ...
    def calculate_adjusted_loss(row):
        ...
        if row["model_epsilon"] < row["exact_epsilon"]:
            if (
                row["exact_epsilon"] - row["model_epsilon"] > 0.5
            ...
            if (
                row["model_epsilon"] - row["exact_epsilon"] > 0.5
            ):  # the values are on opposite sides of the cycle
                adjusted_loss = -abs(adjusted_loss)  # it's an underestimation
            else:
                adjusted_loss = abs(adjusted_loss)  # it's an overestimation
...
def plot_average_cmae(
    df: pd.DataFrame,
    title: str,
    special_ranges: list = SPECIAL_RANGES,
    threshold: float = 0.1,
):
    indices = []
    cmaes = []
...
def plot_ranged_loss(df: pd.DataFrame, title: str):
    ...
    for x in range(0, 10):
        sample = df[
            (df["exact_epsilon"] >= x / 10) & (df["exact_epsilon"] < (x + 1) / 10)
        ]
        cmae = sample["cmae"].mean()
        cmaes.append(cmae)
        indices.append(x / 10)
...
def plot_epsilon_distribution_ax(df, title, ax, color="red"):
    ax.hist(
        df["epsilon"], bins=10, range=(0, 1), color=color, alpha=0.5, edgecolor="black"
    )
    ax.set_title(title)
...
def plot_average_cmae_ax(
    df: pd.DataFrame, title: str, ax, special_ranges: list = SPECIAL_RANGES
):
    indices = []
    cmaes = []
    for x in range(0, 99000, 100):
...
def plot_subarea(df: pd.DataFrame, title: str, start: int, samples: int = 1000):
    ...
    plt.ylabel("Y")
    plt.plot(
        df["filenr"][start : start + samples],
        df["model_epsilon"][start : start + samples],
    )
    plt.show()
...
def plot_subarea_ax(df: pd.DataFrame, title: str, ax, start: int, samples: int = 1000):
    ...
    ax.plot(
        df["filenr"][start : start + samples],
        df["exact_epsilon"][start : start + samples],
        c="b",
        alpha=0.5,
        linestyle="--",
    )
